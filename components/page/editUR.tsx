...
function getStatusClasses(status: GoalProgressStatus) {
	if (status === "Not Started") return "bg-red-100 text-red-800 hover:bg-red-200"
	if (status === "In Progress") return "bg-yellow-100 text-yellow-800 hover:bg-yellow-200"
...
export default function EditURPage({ urId }: { urId: string }) {
	const { t } = useLanguageContext()
	const router = useRouter()

	const [ur, setUr] = useState<GoalItem | null>(null)
	...
	useEffect(() => {
		const loadData = async () => {
			try {
				...
				const upcomingResponsibility = await handleGetGoalById("Upcoming Responsibility", urId)

				if (!upcomingResponsibility) {
					setLoadError(t("goalNotFound"))
					return
				}

				setUr(upcomingResponsibility)
				setEditName(upcomingResponsibility.name ?? "")
				setEditDue(toDatetimeLocal(upcomingResponsibility.to))

				const linkedDailyTasks = upcomingResponsibility.ur_id
					? await handleGetLinkedGoals("Daily Tasks", "ur_id", upcomingResponsibility.ur_id)
					: []
				setDailyTasks(linkedDailyTasks)

				const nextStatuses: Record<string, GoalProgressStatus> = {}
				for (const goal of linkedDailyTasks) {
					const goalId = goal._id ?? goal.id ?? ""
					if (goalId) nextStatuses[goalId] = goal.completed
				}
	...
	const handleSave = async () => {
		...
		try {
			await handleSaveGoalEdits("Upcoming Responsibility", urId, editName.trim(), new Date(editDue))
			setUr((previous) =>
				previous
					? { ...previous, name: editName.trim(), to: new Date(editDue) }
					: previous
			)
	...
	const handleDelete = async () => {
		...
		setIsDeleting(true)
		try {
			await handleDeleteTask("Upcoming Responsibility", urId)
	...
	const handleToggleStatus = async (goal: GoalItem, goalType: string) => {
		...
		setGoalStatuses((previous) => ({ ...previous, [goalId]: nextStatus }))
		...
		try {
			...
			setGoalStatuses((previous) => ({ ...previous, [goalId]: currentStatus }))
		}
	}
	...
	return (
		<div className="mx-auto w-full max-w-4xl space-y-6 p-4 md:p-6">
			...
			<GoalTableSection
				title={t("linkedDailyTasks")}
				goals={dailyTasks}
				goalType="Daily Tasks"
				showFrom
