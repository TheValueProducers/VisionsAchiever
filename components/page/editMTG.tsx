...
export default function EditMTGPage({ mtgId }: { mtgId: string }) {
	const { t } = useLanguageContext()
	const router = useRouter()

	const [mtg, setMtg] = useState<GoalItem | null>(null)
	...
	useEffect(() => {
		const loadData = async () => {
			try {
				...
				const midTermGoal = await handleGetGoalById("Mid Term Goal", mtgId)
				...
				setMtg(midTermGoal)
				setEditName(midTermGoal.name ?? "")
				setEditDue(toDatetimeLocal(midTermGoal.to))

				const linkedURs = midTermGoal.mtg_id
					? await handleGetLinkedGoals("Upcoming Responsibility", "mtg_id", midTermGoal.mtg_id)
					: []
				setUpcomingResponsibilities(linkedURs)

				const nextStatuses: Record<string, GoalProgressStatus> = {}
				for (const goal of linkedURs) {
					const goalId = goal._id ?? goal.id ?? ""
	...
	const handleSave = async () => {
		...
		try {
			await handleSaveGoalEdits("Mid Term Goal", mtgId, editName.trim(), new Date(editDue))
			setMtg((previous) =>
				previous
	...
	const handleDelete = async () => {
		...
		setIsDeleting(true)
		try {
			await handleDeleteTask("Mid Term Goal", mtgId)
	...
	return (
		<div className="mx-auto w-full max-w-4xl space-y-6 p-4 md:p-6">
			...
			<GoalTableSection
				title={t("linkedUpcomingResponsibilities")}
				goals={upcomingResponsibilities}
				goalType="Upcoming Responsibility"
				showCategory
